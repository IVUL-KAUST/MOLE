...
all_files = glob.glob(f"{base_path}/{distilled_model}-results.json")
...
dataset = Dataset.from_list([{"path": file} for file in all_files])
dataset = dataset.map(create_prompts, batched=True, batch_size=16, num_proc=16)
dataset = dataset.map(lambda x: {"formatted_chat": tokenizer.apply_chat_template(x["chat"], tokenize=False, add_generation_prompt=False)})

dataset.push_to_hub("IVUL-KAUST/mole_synth_dataset", private=True)