...
def process_subsets(metric_results, subset):
    ...
    for model_name in metric_results:
        ...
        for prediction in predictions:
            ...
            results_per_model[model_name].append([prediction[column] for column in headers])
...
def plot_subsets():
    ...
    for json_file in json_files:
        ...
        metric_results[model_name].append(get_predictions(gold_results['metadata'], results['metadata']))

    for subset in evaluation_subsets:
        headers = evaluation_subsets[subset]
        headers = ["MODEL"] + [h.capitalize() for h in headers] + ["AVERAGE"] #capitalize each letter in header name
